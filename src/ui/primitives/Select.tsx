    ...
    >
      <div className={cx('grid gap-1.5', className)}>
        ...
        <BaseSelect.Trigger
          ...
          className={unstyled ? triggerClassName : cx(
            'inline-flex h-9 min-w-36 items-center justify-between gap-3 rounded-md border border-curator-border bg-curator-bg-panel px-3 text-sm text-curator-text outline-none transition-colors data-[popup-open]:border-curator-border-strong focus-visible:ring-2 focus-visible:ring-white/20 disabled:opacity-50',
            triggerClassName
          )}
        >
          <BaseSelect.Value className={valueClassName} placeholder={placeholder}>
            {(selectedValue: string | null) => options.find((option) => option.value === selectedValue)?.label ?? placeholder}
          </BaseSelect.Value>
          {unstyled ? (
            <BaseSelect.Icon className="custom-select-trigger-arrow" aria-hidden="true" />
          ) : (
            <BaseSelect.Icon>
      ...
      <BaseSelect.Portal>
        <BaseSelect.Positioner className={positionerClassName} sideOffset={6} alignItemWithTrigger={false}>
          <BaseSelect.Popup className={unstyled ? popupClassName : cx(
            ...
            <BaseSelect.List className={listClassName ?? (unstyled ? undefined : 'contents')}>
              {options.map((option) => (
                <BaseSelect.Item
                  key={option.value}
                  value={option.value}
                  ...
                  className={itemClassNameResolver ?? cx(
                    'flex cursor-default items-center justify-between gap-3 rounded px-2.5 py-2 text-sm outline-none data-[highlighted]:bg-curator-muted data-[disabled]:opacity-45',
                    itemClassName
                  )}
                >
