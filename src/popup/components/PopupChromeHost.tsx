...
let popupSearchHelpPopoverComponent: PopupSearchHelpPopoverComponent | null = null
let popupSearchHelpPopoverPromise: Promise<PopupSearchHelpPopoverComponent> | null = null

function loadPopupSearchHelpPopover(): Promise<PopupSearchHelpPopoverComponent> {
  if (popupSearchHelpPopoverComponent) {
    return Promise.resolve(popupSearchHelpPopoverComponent)
  }
  popupSearchHelpPopoverPromise ||= import('./PopupSearchHelpPopover').then((module) => {
    popupSearchHelpPopoverComponent = module.PopupSearchHelpPopover
    return module.PopupSearchHelpPopover
  })
  return popupSearchHelpPopoverPromise
}
...
function SearchHelpControl() {
  ...
  const [Popover, setPopover] = useState<PopupSearchHelpPopoverComponent | null>(
    popupSearchHelpPopoverComponent
  )
  const pendingOpenRef = useRef(false)

  const mountPopover = () => {
    void loadPopupSearchHelpPopover().then((Component) => {
  ...
  useEffect(() => {
    if (!Popover || !pendingOpenRef.current) {
      return
    }
