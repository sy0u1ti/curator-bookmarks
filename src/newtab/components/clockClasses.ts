...
const CLOCK_BASE_CLASS = 'newtab-clock grid w-fit max-w-[min(100%,1229px)] min-h-[calc(38px*var(--clock-scale))] auto-cols-max grid-flow-col items-center justify-center gap-[calc(10px*var(--clock-scale))] rounded-[calc(12px*var(--clock-scale))] border border-[var(--newtab-glass-stroke)] [border-width:var(--newtab-glass-stroke-width)] bg-[var(--newtab-glass-bg-hero)] px-[calc(11px*var(--clock-scale))] py-[calc(7px*var(--clock-scale))] text-center text-[var(--ui-text-primary)] shadow-none [filter:var(--newtab-glass-drop)] select-none [-webkit-backdrop-filter:var(--newtab-glass-filter-hero)] [backdrop-filter:var(--newtab-glass-filter-hero)]'
