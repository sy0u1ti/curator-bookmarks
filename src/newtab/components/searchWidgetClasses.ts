...
export const SEARCH_FORM_CLASS = 'newtab-search group relative z-[1] flex h-[var(--search-height)] w-full items-center gap-2 border-0 bg-transparent py-0 pr-2 pl-4 outline-none [--search-height:40px] [--search-width:44vw]'
export const SEARCH_FORM_PANEL_OPEN_CLASS = 'is-panel-open'
export const SEARCH_ICON_CLASS = 'newtab-search-icon pointer-events-none shrink-0 text-[rgba(245,245,247,0.9)] transition-colors duration-[var(--ui-motion-fast)] ease-[var(--ui-ease-standard)] group-focus-within:text-[var(--ui-text-primary)] motion-reduce:transition-none'
export const SEARCH_INPUT_CLASS = 'newtab-search-input h-full min-w-0 flex-1 appearance-none rounded-none border-0 bg-transparent px-0.5 text-[15px] font-medium leading-normal text-[var(--ui-text-primary)] caret-[var(--ui-text-primary)] outline-0 placeholder:text-[rgba(245,245,247,0.9)] [&::-webkit-search-cancel-button]:hidden [&::-webkit-search-cancel-button]:appearance-none [&::-webkit-search-decoration]:hidden [&::-webkit-search-decoration]:appearance-none'
...
const SEARCH_CHIP_BUTTON_BASE_CLASS = 'newtab-search-chip-button inline-flex h-[30px] min-h-0 w-auto max-w-none shrink-0 items-center justify-center gap-[5px] whitespace-nowrap rounded-[var(--ui-radius-control)] border border-transparent bg-transparent px-3 text-xs font-medium leading-none tracking-[0] text-[var(--ui-text-secondary)] transition-colors duration-[var(--ui-motion-fast)] ease-[var(--ui-ease-standard)] enabled:hover:text-[var(--ui-text-primary)] focus-visible:text-[var(--ui-text-primary)] focus-visible:[outline:3px_solid_var(--ui-focus-ring-soft)] focus-visible:outline-offset-0 motion-reduce:transition-none'
const SEARCH_NATURAL_ACTIVE_CLASS = 'text-[var(--ui-text-inverse)] enabled:hover:text-[var(--ui-text-inverse)] focus-visible:text-[var(--ui-text-inverse)]'
const SEARCH_NATURAL_FALLBACK_CLASS = 'text-[var(--ui-warning)] enabled:hover:text-[var(--ui-warning)] focus-visible:text-[var(--ui-warning)]'
...
export const SEARCH_ENGINE_BUTTON_CLASS = cx('newtab-search-engine', SEARCH_CHIP_BUTTON_BASE_CLASS, 'min-w-13 disabled:cursor-default disabled:opacity-40')
...
export const SEARCH_ENGINE_MENU_HINT_CLASS = 'newtab-search-engine-menu-hint px-2 pt-1 pb-0.5 text-xs leading-[1.4] text-[rgba(245,245,247,0.9)]'

export const SEARCH_PANEL_CLASS = 'newtab-search-suggestions-panel relative z-[1] grid min-w-0 w-full'
export const SEARCH_CHIPS_CLASS = 'newtab-search-chips flex flex-wrap gap-[5px] px-1.5 py-0.5 empty:hidden'
export const SEARCH_SECTION_LABEL_CLASS = 'newtab-search-section-label px-2 pt-0.5 pb-px text-xs font-bold leading-[1.3] text-[rgba(245,245,247,0.9)] empty:hidden'
export const SEARCH_SUGGESTIONS_CLASS = 'newtab-search-suggestions grid gap-[3px]'
export const SEARCH_HINT_CLASS = 'newtab-search-hint px-2 pt-[3px] pb-0.5 text-xs font-[560] leading-[1.4] text-[rgba(245,245,247,0.9)] empty:hidden'
const SEARCH_CHIP_CLASS = 'newtab-search-chip inline-flex min-h-[22px] max-w-full items-center overflow-hidden text-ellipsis whitespace-nowrap rounded-[var(--ui-radius-pill)] border border-[var(--ui-divider)] bg-[rgba(245,245,247,0.055)] px-[7px] text-xs font-[680] leading-[1.2] text-[var(--ui-text-secondary)]'
...
const SEARCH_SUGGESTION_MARK_CLASS = 'newtab-search-suggestion-mark grid h-[30px] w-[30px] place-items-center rounded-lg border border-[rgba(245,245,247,0.1)] bg-[rgba(245,245,247,0.07)] text-xs font-extrabold leading-none text-[rgba(245,245,247,0.9)]'
