...
export function getSpeedDialPinActionCopy(isPinned: boolean, workspaceName = '当前场景'): {
  label: string
  status: string
  ariaLabel: string
} {
  return isPinned
    ? {
        label: '取消固定',
        status: `已从${workspaceName}取消固定`,
        ariaLabel: `从${workspaceName}取消固定书签`
      }
    : {
        label: `固定到${workspaceName}`,
        status: `已固定到${workspaceName}`,
        ariaLabel: `固定书签到${workspaceName}`
      }
}
