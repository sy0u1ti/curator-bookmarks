...
- 当前集成代码状态：在 `e170e59` 基础上继续补齐标签与备份数据按钮上下文；报告最新提交以 `git log -1 --oneline` 为准。
...
- [低] UI/可访问性：popup 文件夹候选容器缺少稳定名称和方向键导航
  ...
  - 影响：筛选、移动、智能保存的文件夹候选容器已有 listbox/tree 语义，但容器和对应搜索框缺少完整稳定名称/关联；筛选 listbox 原本还需要连续 Tab 浏览候选文件夹，筛选后缺少稳定的当前选项焦点。
  - 建议：为三个候选容器和搜索框添加稳定 `aria-label`，并用 `aria-controls` 关联搜索框与结果列表；筛选搜索框支持 ArrowDown/ArrowUp 进入首尾候选项，列表内支持 ArrowUp/ArrowDown/Home/End/Escape，候选项使用 roving `tabIndex`。
  - 处理：已完成，popup 文件夹候选容器和搜索框均有稳定名称，搜索框关联各自结果列表；筛选文件夹列表支持方向键进入、循环移动、Home/End 跳转和 Escape 回到搜索框。

- [低] UI/可访问性：popup 文件夹展开/折叠按钮名称缺少文件夹上下文
  - 位置：`src/popup/popup.ts` / `renderFolderNode`、`renderMoveFolderNode`、`renderSmartFolderNode`
  - 影响：书签树、移动目标树和保存目标树中会连续出现多个“展开文件夹/折叠文件夹”按钮，辅助技术用户难以确认将展开或折叠哪个文件夹。
  - 建议：展开/折叠按钮的 `aria-label` 包含当前文件夹标题或路径，并限制长路径朗读长度。
  - 处理：已完成，popup 三处文件夹树 toggle 现在使用 `展开文件夹：${folderPath}` / `折叠文件夹：${folderPath}`。
...
- [低] UI/可访问性：Dashboard 批量选择按钮名称缺少区域上下文
  - 位置：`src/options/options.html` / `dashboard-select-visible-top`、`dashboard-clear-selection`、`dashboard-move-selection`、`dashboard-delete-selection`
  - 影响：Dashboard 工具栏和已选区中的“选择当前可见、清空选择、批量移动、批量删除”按钮依赖视觉区域表达对象，辅助技术用户按按钮浏览时难以确认这些动作针对 Dashboard 书签。
  - 建议：为 Dashboard 批量按钮补充区域和已选书签上下文。
  - 处理：已完成，相关按钮现在使用 `选择当前可见的 Dashboard 书签`、`批量删除 Dashboard 已选书签` 等稳定名称。

- [低] UI/可访问性：Dashboard 标签编辑动作按钮名称缺少当前书签上下文
  - 位置：`src/options/options.html` / `dashboard-tag-editor-clear-ai`、`dashboard-tag-editor-regenerate-ai`、`dashboard-tag-editor-save`、`data-dashboard-action="close-tag-editor"`
  - 影响：Dashboard 标签编辑弹窗里的“清除 AI 标签、重新生成 AI 标签、取消、保存标签”按钮依赖弹窗标题表达对象，辅助技术用户按按钮浏览时难以确认这些动作针对当前 Dashboard 书签标签。
  - 建议：为标签编辑动作按钮补充当前 Dashboard 书签标签上下文。
  - 处理：已完成，相关按钮现在使用 `清除当前 Dashboard 书签的 AI 标签`、`保存当前 Dashboard 书签标签` 等稳定名称。

- [低] UI/可访问性：options 弹窗页脚按钮名称缺少弹窗上下文
  - 位置：`src/options/options.html` / `cancel-delete-modal`、`confirm-delete-modal`、`cancel-confirm-modal`、`confirm-modal-confirm`、`cancel-move-modal`、`cancel-scope-modal`、`cancel-ai-model-modal`、`save-ai-model-modal`、`ai-model-picker-fetch`、`ai-model-picker-manage`、`cancel-ai-model-picker-modal`
  - 影响：删除确认、通用确认、批量移动、筛选文件夹、自定义模型和 AI 模型选择弹窗中的“取消、确认、保存、关闭、获取模型”等按钮依赖弹窗标题表达对象，辅助技术用户按按钮浏览时难以确认当前按钮对应哪个弹窗或数据范围。
  - 建议：为弹窗页脚按钮补充对应弹窗和动作对象上下文。
  - 处理：已完成，相关按钮现在使用 `确认批量删除高置信异常书签`、`保存自定义模型列表`、`关闭 AI 模型选择弹窗` 等稳定名称。

- [低] UI/可访问性：可用性检测批量选择按钮名称缺少区域上下文
  - 位置：`src/options/options.html` / `availability-clear-selection`、`availability-selection-retest`、`availability-selection-promote`、`availability-selection-demote`、`availability-selection-move`、`availability-selection-ignore-*`、`availability-selection-delete`
  - 影响：可用性检测已选区中的“重新测试、移入高置信异常、批量移动、忽略、删除”等按钮依赖视觉区域表达对象，辅助技术用户按按钮浏览时难以确认这些动作针对可用性检测已选书签。
  - 建议：为可用性检测批量按钮补充区域和已选书签上下文。
  - 处理：已完成，相关按钮现在使用 `重新测试可用性检测已选书签`、`批量删除可用性检测已选书签` 等稳定名称。

- [低] UI/可访问性：可用性检测分区与历史按钮名称缺少区域上下文
  - 位置：`src/options/options.html` / `availability-select-all-review`、`availability-select-all-failed`、`delete-failed-bookmarks`、`availability-clear-history`
  - 影响：可用性检测分区中的“全选本区、批量删除”和检测历史中的“清空历史日志”依赖视觉区域表达对象，辅助技术用户按按钮浏览时难以确认这些动作针对低置信、高置信还是历史记录。
  - 建议：为分区选择、分区删除和历史清空按钮补充置信分区或历史上下文。
  - 处理：已完成，相关按钮现在使用 `全选低置信异常书签`、`批量删除高置信异常书签`、`清空可用性检测历史日志` 等稳定名称。
...
- [低] UI/可访问性：回收站批量按钮名称缺少区域上下文
  - 位置：`src/options/options.html` / `recycle-clear-selection`、`recycle-restore-selection`、`recycle-clear-selected`、`recycle-select-all`、`recycle-clear-all`
  - 影响：回收站已选区和条目区中的“清空选择、批量恢复、清除所选、全选本区、清空回收站”等按钮依赖视觉区域表达对象，辅助技术用户按按钮浏览时难以确认这些动作针对回收站记录。
  - 建议：为回收站批量按钮补充区域和已选对象上下文。
  - 处理：已完成，相关按钮现在使用 `批量恢复回收站已选书签`、`清空全部回收站记录` 等稳定名称。
...
- [低] UI/可访问性：重定向更新批量按钮名称缺少区域上下文
  - 位置：`src/options/options.html` / `redirect-clear-selection`、`redirect-batch-update`、`redirect-delete-selection`、`redirect-select-all`、`redirect-delete-all`
  - 影响：重定向更新页中的“清空选择、批量更新最终 URL、删除所选、全选本区、批量删除本区”等按钮依赖视觉区域表达对象，辅助技术用户按按钮浏览时难以确认这些动作针对待更新重定向书签。
  - 建议：为重定向批量按钮补充区域和已选书签上下文。
  - 处理：已完成，相关按钮现在使用 `批量更新重定向已选书签为最终 URL`、`批量删除待更新重定向书签` 等稳定名称。
...
- [低] UI/可访问性：书签智能分析批量选择按钮名称缺少区域上下文
  - 位置：`src/options/options.html` / `ai-select-all`、`ai-select-high-confidence`、`ai-clear-selection`、`ai-move-selection-to-suggested`、`ai-apply-selection`
  - 影响：书签智能分析已选区中的“全选、清空选择、移动至推荐文件夹、应用所选建议”等按钮依赖视觉区域表达对象，辅助技术用户按按钮浏览时难以确认这些动作针对智能分析建议。
  - 建议：为智能分析批量按钮补充区域和已选建议上下文。
  - 处理：已完成，相关按钮现在使用 `全选书签智能分析结果`、`应用书签智能分析已选建议` 等稳定名称。

- [低] UI/可访问性：历史与标签清理按钮名称缺少对象上下文
  - 位置：`src/options/options.html` / `bookmark-add-history-clear`、`ai-clear-filters`、`ai-tag-clear`
  - 影响：自动分析历史、书签智能分析筛选和标签数据区域中的“清空自动分析添加历史、清空筛选、清空标签数据”等按钮依赖邻近区域表达对象，辅助技术用户按按钮浏览时难以确认将清空哪类数据。
  - 建议：为历史、筛选和标签清理按钮补充明确对象名称。
  - 处理：已完成，相关按钮现在使用 `清空自动分析添加历史记录`、`清空书签智能分析筛选条件`、`清空全部书签标签数据` 等稳定名称。

- [低] UI/可访问性：标签与备份数据按钮名称缺少数据范围上下文
  - 位置：`src/options/options.html` / `ai-tag-export`、`ai-tag-import`、`backup-export`、`backup-import`、`backup-restore-tags`、`backup-restore-newtab`、`backup-restore-safe-full`
  - 影响：标签数据和完整备份区域中的“导出、导入、恢复”按钮依赖区域标题表达数据范围，辅助技术用户按按钮浏览时难以确认当前动作针对书签标签数据、完整备份还是备份预览中的某个恢复范围。
  - 建议：为标签导入导出、完整备份导入导出和恢复预览按钮补充数据范围上下文。
  - 处理：已完成，相关按钮现在使用 `导出书签标签数据`、`导入完整备份并预览`、`从备份预览恢复全部可安全恢复的数据` 等稳定名称。

- [低] UI/可访问性：重复书签策略按钮名称缺少结果上下文
  - 位置：`src/options/options.html` / `duplicate-strategy-controls`
  - 影响：重复书签策略栏中的“保留最新、保留最早、保留路径最短、保留有标签”等按钮依赖区域标题表达对象，辅助技术用户按按钮浏览时难以确认这些策略会作用于重复书签当前结果。
  - 建议：为重复书签保留策略按钮补充重复书签和策略对象上下文。
  - 处理：已完成，相关按钮现在使用 `保留重复书签中的最新项`、`保留重复书签中路径最短的项` 等稳定名称。
...
- [低] UI/可访问性：重复书签批量按钮名称缺少区域上下文
  - 位置：`src/options/options.html` / `duplicate-clear-selection`、`duplicate-delete-selection`
  - 影响：重复书签已选区中的“清空选择、移入回收站”按钮依赖视觉区域表达对象，辅助技术用户按按钮浏览时难以确认这些动作针对重复书签已选项。
  - 建议：为重复书签批量按钮补充区域和已选对象上下文。
  - 处理：已完成，相关按钮现在使用 `清空重复书签已选项`、`将重复书签已选项移入回收站` 等稳定名称。

- [低] UI/可访问性：忽略规则删除/清空按钮名称缺少规则上下文
  - 位置：`src/options/sections/ignore.ts` / `buildIgnoreRuleCard`
  - 影响：书签、域名、文件夹三类忽略规则中重复出现“删除规则”和“清空本类”按钮，辅助技术用户难以确认将删除哪条规则或清空哪一类规则。
  - 建议：删除按钮的 `aria-label` 包含当前忽略规则名称、文件夹路径或域名；清空按钮的 `aria-label` 包含规则类别。
  - 处理：已完成，删除按钮现在使用 `删除忽略规则：${rule}`，清空按钮使用 `清空按书签/域名/文件夹忽略规则`，并限制长规则名称朗读长度。
...
- [低] UI/可访问性：newtab 设置抽屉原生控件缺少稳定可访问名称
  - 位置：`src/newtab/newtab.html` / 背景、书签卡片、时间、搜索栏设置控件
  - 影响：背景类型、蒙版样式、时间显示、时区、日期格式、时间密度、默认搜索引擎、背景图片链接、搜索栏占位符和多组 `range` 滑块等控件主要依赖相邻视觉文字；其中滑块视觉标签还包含动态数值，辅助技术或自动化可访问性检查不一定能稳定识别用途。
  - 建议：为这些原生 `select`/`input` 添加稳定 `aria-label`，并用静态测试防止回退。
  - 处理：已完成，相关 select、文本输入和 range 滑块均已暴露稳定中文可访问名称。

- [低] UI/可访问性：newtab 删除提示动作按钮名称缺少书签上下文
  - 位置：`src/newtab/newtab.ts` / `renderDeleteToast`
  - 影响：删除书签后的 toast 中“撤销”和“回收站”按钮依赖邻近文本表达对象；辅助技术用户按按钮浏览时难以确认操作针对哪条刚删除的书签。
  - 建议：为 toast 动作按钮生成包含书签标题的 `aria-label`，长标题截断。
  - 处理：已完成，删除提示动作按钮使用 `撤销删除：${title}` 和 `打开回收站查看：${title}`。

- [低] UI/可访问性：newtab 书签编辑菜单项名称缺少书签上下文
  - 位置：`src/newtab/newtab.ts` / `renderBookmarkMenu`、`createMenuAction`
  - 影响：书签设置弹窗里的“固定到常用、复制链接、删除链接、刷新图标、保存更改”等 menuitem 只暴露通用动作名，辅助技术用户难以确认动作作用于哪条书签。
  - 建议：编辑现有书签时为菜单项生成“动作 + 书签标题”的 `aria-label`，新增书签菜单保持原有简短名称。
  - 处理：已完成，newtab 书签编辑菜单项现在使用 `复制书签链接：${title}`、`删除书签：${title}`、`保存书签更改：${title}` 等上下文化名称。
...
8. popup 文件夹候选容器缺少稳定名称和方向键导航
   ...
   - 现象描述：三个文件夹候选容器声明为 listbox/tree，但容器和搜索框缺少完整稳定名称与控制关系；筛选文件夹结果列表还缺少 roving focus 和方向键/Home/End/Escape 行为。
   - 对用户的影响：辅助技术用户难以直接区分“筛选候选”“移动目标”“保存目标”三个区域，也难以确认搜索框更新的是哪个候选列表；键盘用户筛选文件夹时需要连续 Tab 才能浏览候选项，筛选后当前候选项上下文不稳定。
   - 严重程度：低
   - 推荐改进方案：为候选容器和搜索框添加稳定 `aria-label`，搜索框用 `aria-controls` 指向对应结果列表；筛选搜索框支持方向键进入首尾候选项，列表支持方向键循环、Home/End 跳转、Escape 回到搜索框。
   - 处理状态：已修复。

9. popup 文件夹展开/折叠按钮可访问名称重复
   - 页面/组件位置：popup 书签树、移动书签弹窗、智能保存文件夹弹窗
   - 现象描述：多层文件夹树中每个展开/折叠 toggle 都重复暴露“展开文件夹”或“折叠文件夹”。
   - 对用户的影响：辅助技术用户浏览文件夹树时，需要额外回读相邻文件夹卡片才能确认将展开或折叠哪个文件夹。
   - 严重程度：低
   - 推荐改进方案：为展开/折叠按钮生成“动作 + 文件夹路径/标题”的可访问名称，长路径截断。
   - 处理状态：已修复。

10. options 范围筛选文件夹搜索框语义不完整且 listbox 缺少方向键导航
...
11. options AI 模型 picker 搜索框语义不完整且 listbox 缺少可访问名称与方向键导航
...
12. options 移动目标文件夹 picker 搜索框语义不完整且缺少 listbox 语义与方向键导航
...
14. Dashboard 批量选择按钮可访问名称缺少区域上下文
   - 页面/组件位置：options dashboard 工具栏和已选区
   - 现象描述：“选择当前可见、清空选择、批量移动、批量删除”等按钮只暴露通用动作名。
   - 对用户的影响：辅助技术用户按按钮浏览时，难以确认这些批量动作针对 Dashboard 当前结果或 Dashboard 已选书签。
   - 严重程度：低
   - 推荐改进方案：为 Dashboard 批量按钮生成带区域和对象的可访问名称，保留原有视觉短文案。
   - 处理状态：已修复。

15. Dashboard 标签编辑动作按钮可访问名称缺少当前书签上下文
   - 页面/组件位置：options dashboard 标签编辑弹窗
   - 现象描述：“清除 AI 标签、重新生成 AI 标签、取消、保存标签”等按钮只通过弹窗上下文表达当前对象。
   - 对用户的影响：辅助技术用户按按钮浏览时，难以确认这些动作针对当前 Dashboard 书签标签，清除和重新生成 AI 标签尤其容易混淆作用范围。
   - 严重程度：低
   - 推荐改进方案：为标签编辑动作按钮生成带当前 Dashboard 书签标签对象的可访问名称，保留原有视觉短文案。
   - 处理状态：已修复。

16. options 弹窗页脚按钮可访问名称缺少弹窗上下文
   - 页面/组件位置：options 删除确认、通用确认、批量移动、筛选文件夹、自定义模型和 AI 模型选择弹窗
   - 现象描述：“取消、确认、保存、关闭、获取模型、设置更多模型”等按钮只通过弹窗标题表达当前动作对象。
   - 对用户的影响：辅助技术用户按按钮浏览时，难以确认当前按钮对应的是删除高置信异常、批量移动、自定义模型列表还是 AI 模型选择弹窗，确认和保存类动作尤其容易混淆范围。
   - 严重程度：低
   - 推荐改进方案：为弹窗页脚按钮生成带弹窗和动作对象的可访问名称，保留原有视觉短文案。
   - 处理状态：已修复。
...
18. 回收站批量按钮可访问名称缺少区域上下文
   - 页面/组件位置：options 回收站已选区和条目区
   - 现象描述：“清空选择、批量恢复、清除所选、全选本区、清空回收站”等按钮只暴露通用动作名。
   - 对用户的影响：辅助技术用户按按钮浏览时，难以确认这些批量动作针对回收站记录还是其他管理页条目，清除类动作尤其需要明确对象范围。
   - 严重程度：低
   - 推荐改进方案：为回收站批量按钮生成带区域和已选对象的可访问名称，保留原有视觉短文案。
   - 处理状态：已修复。
...
20. 重定向更新批量按钮可访问名称缺少区域上下文
   - 页面/组件位置：options 重定向更新已选区和待更新区
   - 现象描述：“清空选择、批量更新最终 URL、删除所选、全选本区、批量删除本区”等按钮只暴露通用动作名。
   - 对用户的影响：辅助技术用户按按钮浏览时，难以确认这些批量动作针对重定向更新结果或已选重定向书签，批量更新和删除类动作尤其容易混淆作用范围。
   - 严重程度：低
   - 推荐改进方案：为重定向批量按钮生成带区域和已选对象的可访问名称，保留原有视觉短文案。
   - 处理状态：已修复。

21. 书签智能分析结果筛选输入语义不完整且动作按钮可访问名称重复
...
22. 书签智能分析批量选择按钮可访问名称缺少区域上下文
   - 页面/组件位置：options 书签智能分析已选区
   - 现象描述：“全选、全选高置信、清空选择、移动至推荐文件夹、应用所选建议”等按钮只暴露通用动作名。
   - 对用户的影响：辅助技术用户按按钮浏览时，难以确认这些批量动作针对书签智能分析结果或已选建议，应用和移动类动作尤其容易混淆作用范围。
   - 严重程度：低
   - 推荐改进方案：为书签智能分析批量按钮生成带区域和已选建议对象的可访问名称，保留原有视觉短文案。
   - 处理状态：已修复。

23. 历史与标签清理按钮可访问名称缺少对象上下文
   - 页面/组件位置：options 自动分析添加历史、书签智能分析筛选、标签数据
   - 现象描述：“清空自动分析添加历史、清空筛选、清空标签数据”等按钮只通过邻近区域说明对象。
   - 对用户的影响：辅助技术用户按按钮浏览时，难以确认当前清空的是添加历史、智能分析筛选条件还是全部标签数据。
   - 严重程度：低
   - 推荐改进方案：为历史、筛选和标签清理按钮生成带对象名称的可访问名称，破坏性标签清空动作尤其需要明确范围。
   - 处理状态：已修复。

24. 标签与备份数据按钮可访问名称缺少数据范围上下文
   - 页面/组件位置：options 标签数据、完整备份、恢复预览
   - 现象描述：“导出标签数据、导入标签数据、导出完整备份、导入并预览、只恢复标签数据、只恢复新标签页设置、恢复全部可安全恢复的数据”等按钮只通过区域标题表达数据范围。
   - 对用户的影响：辅助技术用户按按钮浏览时，难以确认当前动作针对书签标签数据、完整备份还是备份预览恢复范围，导入和恢复类动作尤其需要明确对象。
   - 严重程度：低
   - 推荐改进方案：为标签导入导出、完整备份导入导出和恢复预览按钮生成带数据范围的可访问名称，保留原有视觉短文案。
   - 处理状态：已修复。

25. 重复书签策略按钮可访问名称缺少结果上下文
   - 页面/组件位置：options 重复书签选择策略栏
   - 现象描述：“保留最新、保留最早、保留路径最短、保留有标签、保留新标签页来源、保留最近访问”等按钮只通过区域标题表达当前结果对象。
   - 对用户的影响：辅助技术用户按按钮浏览时，难以确认这些策略会作用于重复书签当前结果，保留策略和移入回收站结果之间的关系不够直接。
   - 严重程度：低
   - 推荐改进方案：为策略按钮生成带重复书签结果对象的可访问名称，保留原有视觉短文案。
   - 处理状态：已修复。
...
27. 重复书签批量按钮可访问名称缺少区域上下文
   - 页面/组件位置：options 重复书签已选区
   - 现象描述：“清空选择、移入回收站”等按钮只暴露通用动作名。
   - 对用户的影响：辅助技术用户按按钮浏览时，难以确认这些批量动作针对重复书签已选项，移入回收站是破坏性动作，尤其需要明确对象范围。
   - 严重程度：低
   - 推荐改进方案：为重复书签批量按钮生成带区域和已选对象的可访问名称，保留原有视觉短文案。
   - 处理状态：已修复。

28. 忽略规则删除/清空按钮可访问名称重复
   - 页面/组件位置：options 忽略规则列表
   - 现象描述：书签、域名、文件夹三类规则里的删除按钮都重复暴露“删除规则”，三类批量清空按钮也都重复暴露“清空本类”。
   - 对用户的影响：辅助技术用户在清理多条忽略规则时，需要额外回读卡片或分区标题确认删除对象或清空类别。
   - 严重程度：低
   - 推荐改进方案：为删除按钮生成“删除忽略规则 + 规则名称/路径/域名”的可访问名称，长文本截断；为三类清空按钮生成类别特定名称。
   - 处理状态：已修复。

29. 文件夹清理建议动作按钮可访问名称重复
...
31. 可用性检测批量选择按钮可访问名称缺少区域上下文
   - 页面/组件位置：options 可用性检测已选区
   - 现象描述：“清空选择、重新测试、移入高置信异常、移入低置信异常、批量移动到文件夹、忽略所选、批量删除所选”等按钮只暴露通用动作名。
   - 对用户的影响：辅助技术用户按按钮浏览时，难以确认这些批量动作针对可用性检测已选书签，忽略和删除类动作尤其容易混淆作用范围。
   - 严重程度：低
   - 推荐改进方案：为可用性检测批量按钮生成带区域和已选对象的可访问名称，保留原有视觉短文案。
   - 处理状态：已修复。

32. 可用性检测分区与历史按钮可访问名称缺少区域上下文
   - 页面/组件位置：options 可用性检测低/高置信结果区与检测历史
   - 现象描述：“全选本区、批量删除、清空历史日志”等按钮只暴露通用动作名。
   - 对用户的影响：辅助技术用户按按钮浏览时，难以确认当前全选的是低置信还是高置信异常，也难以区分删除高置信异常和清空检测历史。
   - 严重程度：低
   - 推荐改进方案：为分区选择、分区删除和历史清空按钮生成带置信分区或历史对象的可访问名称。
   - 处理状态：已修复。

33. newtab 候选文件夹搜索框可访问名称不稳定
...
34. newtab 候选文件夹 listbox 可访问名称不稳定
   - 页面/组件位置：newtab 设置抽屉 / 文件夹来源候选列表
...
35. newtab 设置抽屉原生控件可访问名称不稳定
   - 页面/组件位置：newtab 设置抽屉 / 背景、书签卡片、时间、搜索栏设置
   - 现象描述：多个原生 `select`/`input` 只靠相邻视觉文字或 placeholder 表达用途，部分 `range` 滑块的视觉标签还包含动态 `<output>` 数值，缺少稳定可访问名称。
   - 对用户的影响：辅助技术用户或自动化可访问性检查进入控件时，可能只能获得控件类型和值，或者听到会随数值变化的控件名称，难以稳定确认正在调整的是背景、卡片、时间还是搜索栏设置。
   - 严重程度：低
   - 推荐改进方案：为背景类型、背景图片链接、蒙版样式、背景蒙版模糊、书签卡片布局滑块、时间字号、默认搜索引擎、搜索栏占位符和搜索栏尺寸/位置/背景滑块补充稳定 `aria-label`。
   - 处理状态：已修复。

36. newtab 删除提示动作按钮可访问名称缺少书签上下文
   - 页面/组件位置：newtab 删除书签后的 toast
   - 现象描述：删除提示里“撤销”和“回收站”按钮重复依赖邻近 toast 文本表达刚删除的书签对象。
   - 对用户的影响：辅助技术用户按按钮浏览时，难以直接确认撤销或打开回收站查看的是哪条刚删除的书签。
   - 严重程度：低
   - 推荐改进方案：为 toast 动作按钮生成“动作 + 书签标题”的可访问名称，并截断过长标题。
   - 处理状态：已修复。

37. newtab 书签编辑菜单项可访问名称重复
   - 页面/组件位置：newtab 书签设置弹窗
   - 现象描述：现有书签编辑菜单中的“固定到常用、复制链接、删除链接、刷新图标、保存更改”等动作只暴露通用菜单项名称。
   - 对用户的影响：辅助技术用户按菜单项浏览时，难以确认当前动作作用于哪条书签，删除和保存类动作尤其容易混淆对象。
   - 严重程度：低
   - 推荐改进方案：为现有书签编辑菜单项生成“动作 + 书签标题”的可访问名称，新增书签菜单保留简短名称。
   - 处理状态：已修复。

38. newtab 主搜索输入缺少 combobox 角色
   - 页面/组件位置：newtab 主搜索框
...
40. newtab 候选文件夹 listbox 缺少方向键导航
   - 页面/组件位置：newtab 设置抽屉 / 文件夹来源候选列表
...
- 集成分支补充优化 / `5a8589c`、`32d636d`、`323898b`、`4699fb9`、`d88164f`、`0e7bd5c`、`b185052`、`87f4f3c`、`d4cb535`、`e33821c`、`71770a7`、`3e1e935`、`77b58d9`、`f43c79b`、`53c771d`、`66bbf0d`、`c634fcb`、`1ac5063`、`15fc795`、`a83c2a8`、`33b2a3a`、`0903ab3`、`500624a`、`a9cbde5`、`77ac5c6`、`3cf2451`、`1a48e09`、`a21eac8`、`29e6a18`、`7ac0f64`、`b7dffb8`、`f971cbe`、`719dffc`、`5a4a21b`、`879af2b`、`dba5aba`、`d01d8b3`、`590311a`、`464cbe7`、`7d80029`、`31dc838`、`f7f3b4c`、`07db61d`、`def28a0`、`87ad604`、`70b6e70`、`cd4fb85`、`c830f52`、`e170e59`、本提交
  - 实现思路：修复 popup 窄视口横向溢出；将 newtab 搜索重 chunk 改为按需加载，并保留轻量同步建议；将 newtab 标签索引读取改为轻量 storage normalizer；内联 newtab loading SVG 和关闭动效 helper；将回收站删除/撤销模块改为按需加载；将启动读书签树改为本页轻量 wrapper，书签移动、编辑、新建、撤销恢复等写操作通过 `bookmarks-api` 动态加载；将 popup 自然语言搜索、智能分类网页内容抽取、AI 设置归一化、AI 响应解析、Inbox 状态模块、回收站事务 helper 和完整内容快照存储模块改为触发对应功能后再加载或通过轻量常量/搜索入口解耦；自动分析失败后按剩余队列重新计算下一次唤醒，移除首屏和后台队列的非必要运行成本；popup 书签树和搜索结果的操作菜单按钮及菜单项使用书签标题生成可访问名称；popup 书签操作菜单支持触发按钮方向键入口和关闭回焦；popup 文件夹候选容器和搜索框补齐稳定名称，搜索框关联对应结果列表，筛选文件夹 listbox 补齐 roving focus 和方向键导航；popup 书签树、移动目标树和保存目标树的展开/折叠按钮加入文件夹路径上下文；options 范围筛选、AI 模型 picker、移动目标文件夹 picker 搜索框补齐稳定名称和结果列表关联；options 范围筛选文件夹 listbox 补齐 roving focus 和方向键导航；options AI 模型 picker listbox 补齐可访问名称、roving focus 和方向键导航；options 移动目标文件夹 picker 补齐 listbox/option 语义与方向键导航；Dashboard 卡片打开、复制、改标签、移动、删除动作、批量选择按钮和标签编辑动作按钮加入 Dashboard/书签上下文；options 删除确认、通用确认、批量移动、筛选文件夹、自定义模型和 AI 模型选择弹窗页脚按钮加入弹窗上下文；可用性检测批量选择、低/高置信分区和历史日志按钮加入区域上下文；回收站选择、恢复和清除控件加入书签标题上下文，回收站批量按钮加入区域和已选记录上下文；重定向结果选择、更新和打开最终链接控件加入书签标题上下文，重定向批量按钮加入区域和已选书签上下文；书签智能分析结果筛选输入补齐稳定名称，结果选择、打开、应用和移动控件加入书签标题上下文，批量选择按钮、筛选清空按钮加入区域和对象上下文；标签数据导入导出、完整备份导入导出和恢复预览按钮加入数据范围上下文；自动分析添加历史清空和标签数据清空按钮加入清理对象上下文；重复书签策略按钮加入当前结果上下文，逐条移入回收站勾选项加入标题和路径上下文，重复书签批量按钮加入区域和已选项上下文；忽略规则删除按钮加入规则名称、路径或域名上下文，忽略规则清空按钮加入类别上下文；文件夹清理预览、执行和拆分撤销按钮加入建议标题和移动数量上下文；可用性检测结果选择、打开和置信分区移动控件加入书签标题与路径上下文；newtab 候选文件夹搜索框加入稳定名称和候选列表关联；newtab 设置抽屉背景、书签卡片、时间、搜索栏原生控件与 range 滑块补齐稳定名称；newtab 删除提示的撤销和回收站按钮、书签编辑菜单项加入书签标题上下文；newtab 主搜索输入补充 combobox 语义；newtab 搜索引擎菜单补齐键盘焦点与方向键导航；newtab 候选文件夹 listbox 补齐 roving focus 和方向键导航，避免重复或无上下文控件名称。
...
- focused newtab 可访问性测试：通过。
  - `npm run test:build && node --test .tmp-test/tests/newtab-a11y.test.js`：7/7 通过。
  - 覆盖 newtab 设置抽屉 select、文本输入和 range 滑块稳定可访问名称、删除提示动作按钮和书签编辑菜单项的书签特定可访问名称、候选文件夹 picker 命名、局部状态区域、启动动效减弱偏好和 Dashboard overlay 焦点管理。
...
- focused Dashboard 可访问性测试：通过。
  - `npm run test:build && node --test .tmp-test/tests/dashboard-selection-a11y.test.js .tmp-test/tests/options-management-ui.test.js`：52/52 通过。
  - 覆盖 Dashboard 选择框、隐藏标签弹层、文件夹筛选 listbox、弹窗页脚按钮，以及卡片动作按钮、批量选择按钮和标签编辑动作按钮的书签/Dashboard 特定可访问名称。
...
- focused 回收站可访问性测试：通过。
  - `npm run test:build && node --test .tmp-test/tests/recycle.test.js .tmp-test/tests/options-management-ui.test.js`：47/47 通过。
  - 覆盖回收站条目选择、恢复和清除控件的书签特定可访问名称、回收站批量按钮的区域特定名称，以及回收站存储序列化和回滚路径。
- focused options 管理 UI 测试：通过。
  - `npm run test:build && node --test .tmp-test/tests/options-management-ui.test.js`：43/43 通过。
  - 覆盖 Dashboard 标签编辑动作按钮、options 弹窗页脚按钮、标签与备份数据按钮、可用性检测批量选择按钮、可用性检测分区/历史按钮、历史与标签清理按钮、书签智能分析批量选择按钮、重定向更新批量按钮、重复书签策略按钮和重复书签批量按钮的区域特定名称，可用性检测结果选择/打开/置信分区移动控件、重定向结果选择/更新/打开最终链接控件、书签智能分析结果筛选输入和选择/打开/应用/移动控件、重复书签逐条移入回收站勾选项的书签特定可访问名称，以及现有 Dashboard、可用性检测和导航 UI 静态约束。
- focused 忽略规则可访问性测试：通过。
  - `npm run test:build && node --test .tmp-test/tests/ignore.test.js`：5/5 通过。
  - 覆盖忽略规则删除按钮的规则特定可访问名称、三类清空按钮的类别特定可访问名称，以及书签、文件夹、域名三类规则上下文和长文本截断。
...
- Dashboard 批量选择按钮加入 Dashboard 区域和已选书签上下文，降低批量移动/删除时的对象混淆风险。
- Dashboard 标签编辑动作按钮加入当前 Dashboard 书签标签上下文，降低清除、重新生成或保存标签时的对象混淆风险。
- options 弹窗页脚按钮加入弹窗和动作对象上下文，降低确认删除、批量移动、自定义模型保存和 AI 模型选择时的对象混淆风险。
- 可用性检测批量选择按钮加入可用性检测区域和已选书签上下文，降低重新测试、移动、忽略或删除时的对象混淆风险。
- 可用性检测分区与历史按钮加入低/高置信和历史日志上下文，降低全选、批量删除或清空历史时的对象混淆风险。
- 书签智能分析批量选择按钮加入智能分析区域和已选建议上下文，降低批量应用或移动建议时的对象混淆风险。
- 历史与标签清理按钮加入清理对象上下文，降低清空自动分析历史、智能分析筛选或全部标签数据时的对象混淆风险。
- 标签与备份数据按钮加入数据范围上下文，降低导入导出或从备份预览恢复数据时的对象混淆风险。
- 回收站条目选择、恢复、清除控件加入书签标题上下文，降低恢复或永久清除时的误操作风险。
- 回收站批量按钮加入回收站区域和已选记录上下文，降低批量恢复或清除记录时的对象混淆风险。
- 重定向结果选择、更新、打开最终链接控件加入书签标题上下文，降低最终 URL 写入时的误操作风险。
- 重定向更新批量按钮加入重定向区域和已选书签上下文，降低批量更新最终 URL 或删除时的对象混淆风险。
- 书签智能分析结果筛选输入加入稳定可访问名称；选择、打开、应用、移动控件加入书签标题上下文，降低筛选和应用 AI 建议或移动书签时的误操作风险。
- 重复书签策略按钮加入当前结果上下文，降低按策略保留重复项时的对象混淆风险。
- 重复书签逐条移入回收站勾选项加入标题和路径上下文，降低逐条调整保留策略时的误操作风险。
- 重复书签批量按钮加入重复书签区域和已选项上下文，降低批量移入回收站时的对象混淆风险。
- 忽略规则删除按钮加入规则名称、文件夹路径或域名上下文，三类清空按钮加入规则类别上下文，降低清理多条忽略规则时的误删风险。
...
- newtab 设置抽屉的背景、书签卡片、时间和搜索栏原生控件及 range 滑块加入稳定可访问名称，降低辅助技术用户调整设置时的上下文丢失风险。
- newtab 删除提示中的撤销和回收站按钮加入刚删除书签标题上下文，降低辅助技术用户撤销删除或打开回收站时的对象混淆风险。
- newtab 书签编辑菜单项加入当前书签标题上下文，降低固定、复制、删除、刷新和保存时的对象混淆风险。
