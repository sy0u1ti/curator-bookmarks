...
- 当前集成代码状态：在 `5a4a21b` 基础上继续补齐 options 忽略规则清空按钮上下文；报告最新提交以 `git log -1 --oneline` 为准。
...
23. newtab 删除提示动作按钮可访问名称缺少书签上下文
...
26. newtab 候选文件夹 listbox 缺少方向键导航
   - 页面/组件位置：newtab 设置抽屉 / 文件夹来源候选列表
...
- focused popup 可访问性测试：通过。
  ...
  - 覆盖 popup 书签操作菜单按钮的书签特定可访问名称、菜单触发按钮方向键入口与关闭回焦、文件夹候选容器与搜索框可访问名称、搜索框结果列表关联、筛选文件夹 listbox 键盘导航、模态 inert、搜索/模态焦点和空状态。
