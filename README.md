...
**Popup 自然语言搜索** — Popup 搜索框旁的 `AI` 按钮可开启自然语言搜索，用于理解“帮我找上周收藏的那个 React 表格教程”这类带意图、时间和同义表达的查询。配置 AI 渠道后会调用 OpenAI 兼容接口改写查询；AI 未配置或不可用时，会回退到本地时间解析、停用词清理和同义词扩展。本地解析支持今天、上周、去年、最近 N 天/周/月、明确日期，以及“不要视频”“-youtube”这类排除条件。
...
- 书签、历史、忽略规则、回收站、Popup 偏好和 AI 设置存储在 `chrome.storage.local`
- API Key 仅保存在扩展本地存储中
