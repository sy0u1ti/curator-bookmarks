...
test('availability results expose decision filters and single-item ignore actions', () => {
  ...
  assert.match(optionsSource, /action === 'ignore-bookmark' \|\| action === 'ignore-domain' \|\| action === 'ignore-folder'/)
  assert.doesNotMatch(optionsSource, /never-check/)
  assert.doesNotMatch(optionsSource, /同“忽略此书签”/)
