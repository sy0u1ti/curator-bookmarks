...
test('newtab defaults to local color background and keeps featured gallery opt-in', () => {
  const html = readProjectFile('src/newtab/newtab.html')
  const script = readProjectFile('src/newtab/newtab.ts')
  const css = readProjectFile('src/newtab/newtab.css')
  ...
  assert.match(html, /<option value="featured">精选图库<\/option>/)
  assert.match(html, /<option value="color" selected>纯色<\/option>/)
  assert.match(html, /id="background-featured-id"[^>]+aria-label="精选图库图片"/)
  assert.match(html, /主动选择后会访问 NASA、Rijksmuseum、The Met 与 Smithsonian 等第三方图片域名并自动缓存。/)
  assert.match(script, /from '\.\/background-gallery\.js'/)
  assert.match(script, /type:\s*'color'/)
...
test('newtab exposes a lazy options dashboard iframe route', () => {
  ...
  assert.match(script, /NEWTAB_DASHBOARD_OPEN_MESSAGE_TYPE/)
  ...
  assert.match(script, /function resetDashboardFrameReady\(\): void/)
  assert.match(script, /function postDashboardOpenMessage\(\): void/)
  assert.match(script, /postMessage\(\s*\{\s*type:\s*NEWTAB_DASHBOARD_OPEN_MESSAGE_TYPE\s*\}/)
...
test('newtab dashboard glass layer only styles the iframe shell', () => {
  ...
  assert.match(newtabCss, /\.dashboard-shortcut-hint\s*\{[\s\S]*?max-width:\s*min\(220px,\s*calc\(100vw - 168px\)\)/)
  assert.match(newtabCss, /\.dashboard-shortcut-hint\s*\{[\s\S]*?text-overflow:\s*ellipsis[\s\S]*?white-space:\s*nowrap/)
...
test('newtab compact viewport preserves dashboard trigger label while clearing the utility stack', () => {
  ...
  assert.match(compactRule, /\.settings-trigger-zone\s*\{[\s\S]*?width:\s*250px/)
...
test('newtab bookmark suggestions are debounced and cached', () => {
  ...
  assert.match(script, /interface SearchSuggestionCacheEntry/)
  assert.match(script, /const searchSuggestionCache = new Map<string, SearchSuggestionCacheEntry>\(\)/)
  assert.match(script, /const naturalSearchSuggestionCache = new Map<string, SearchSuggestionCacheEntry>\(\)/)
  assert.match(script, /const SEARCH_SUGGESTION_CACHE_TTL_MS = 2 \* 60 \* 1000/)
...
test('newtab releases transient runtime work and ages search suggestion caches', () => {
  const script = readProjectFile('src/newtab/newtab.ts')
  const cleanupBody = getFunctionBody(script, 'cleanupNewTabRuntime')
  const cacheBody = getFunctionBody(script, 'setSearchSuggestionCacheEntry')
  const pruneBody = getFunctionBody(script, 'pruneSearchSuggestionCache')

  assert.match(script, /const SEARCH_SUGGESTION_CACHE_TTL_MS = 2 \* 60 \* 1000/)
  assert.match(cacheBody, /updatedAt: Date\.now\(\)/)
  assert.match(cacheBody, /promise\.catch\(\(\) => \{[\s\S]*?cache\.delete\(cacheKey\)/)
  assert.match(pruneBody, /now - entry\.updatedAt > SEARCH_SUGGESTION_CACHE_TTL_MS/)
  assert.match(script, /window\.addEventListener\('pagehide', cleanupNewTabRuntime\)/)
  assert.match(cleanupBody, /window\.clearTimeout\(clockTimer\)/)
  assert.match(cleanupBody, /window\.cancelAnimationFrame\(deferredRenderFrame\)/)
  assert.match(cleanupBody, /removeBookmarkDragGhost\(\)/)
  assert.match(cleanupBody, /removeFolderDragGhost\(\)/)
  assert.match(cleanupBody, /setActiveBackgroundObjectUrl\(''\)/)
  assert.match(cleanupBody, /searchSuggestionCache\.clear\(\)/)
  assert.match(cleanupBody, /naturalSearchSuggestionCache\.clear\(\)/)
...
test('newtab search suggestions explain bookmark enter behavior and empty web search fallback', () => {
  ...
  assert.match(script, /网页搜索已关闭，可在设置中重新启用/)
  assert.match(script, /suggestionsHint\.textContent = getSearchSuggestionHintText\(\)/)
