...
test('options dashboard entry keeps the settings-page dashboard instead of redirecting to newtab', () => {
  ...
  assert.match(optionsSource, /from '\.\/sections\/dashboard-lazy\.js'/)
  assert.match(optionsSource, /hydrateAvailabilityCatalog\(\{ analyzeFolderCleanup: !IS_OPTIONS_DASHBOARD_EMBED_MODE \}\)/)
  assert.match(optionsSource, /await hydrateAvailabilityCatalog\(\{ analyzeFolderCleanup: !IS_OPTIONS_DASHBOARD_EMBED_MODE \}\)\s+if \(normalizeSectionKey\(getCurrentSectionKey\(\)\) === 'dashboard'\) \{\s+await hydrateDashboardSpeedDialState\(\)\s+renderDashboardSectionIfVisible\(\)\s+\}/)
