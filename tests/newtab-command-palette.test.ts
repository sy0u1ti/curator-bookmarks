...
test('dashboard shortcut uses Cmd or Ctrl K and avoids editable targets', () => {
  assert.equal(shouldOpenDashboardFromKeydown({ key: 'k', metaKey: true }), true)
  assert.equal(shouldOpenDashboardFromKeydown({ key: 'k', ctrlKey: true }), true)
...
test('command palette shortcut is no longer bound to Cmd or Ctrl K', () => {
  assert.equal(shouldOpenCommandPaletteFromKeydown({ key: 'k', metaKey: true }), false)
  assert.equal(shouldOpenCommandPaletteFromKeydown({ key: 'k', ctrlKey: true }), false)
